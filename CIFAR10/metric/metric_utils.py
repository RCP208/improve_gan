...
class MetricOptions:
    def __init__(self, dataset=None, G=None, G_kwargs={}, num_gpus=1, rank=0, device=None, progress=None,
                 zdist=None, ydist=None, cache=True, run_dir=None, cur_nimg=None, snapshot_pkl=None):
...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, swav=False, sfid=False,  **stats_kwargs):
    ...
    for images, _labels in tqdm(torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs)):
        ...
        images = (images * 128 + 127.5).clamp(0, 255).to(torch.uint8)
        if images.shape[1] == 1:
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, swav=False, sfid=False, **stats_kwargs):
    ...
    # fix dropout in eval mode
    def apply_dropout(m):
        if type(m) == nn.Dropout:
            m.train()
    G.apply(apply_dropout)  # TODO: apply dropout in eval
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            z = opts.zdist.sample((batch_gen, ))
            y = opts.ydist.sample((batch_gen, ))
            img = G(z, y)
            img = (img * 128 + 127.5).clamp(0, 255).to(torch.uint8)
            images.append(img)
