...
import timm
from gan_training.models.blocks import FeatureFusionBlock
from gan_training.models.constants import NORMALIZED_INCEPTION, NORMALIZED_IMAGENET, NORMALIZED_CLIP, VITS
...
def _make_projector(im_res, cout, proj_type, expand=False):
    ...
    pretrained_cfg_overlay = dict(file='/home/user/Projects/GAN_PID-master/tf_efficientnet_lite0-0aa007d2.pth')
    model = timm.create_model('tf_efficientnet_lite0', pretrained=True, pretrained_cfg_overlay=pretrained_cfg_overlay)
    ...
    pretrained.CHANNELS = calc_channels(pretrained) # 计算预训练网络四部分的输出通道数

    if proj_type == 0: return pretrained, None

    ### Build CCM
    scratch = nn.Module()
    scratch = _make_scratch_ccm(scratch, in_channels=pretrained.CHANNELS, cout=cout, expand=expand) # 4个随机初始化权重的1×1卷积，输出通道数翻倍
...
class F_RandomProj(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.cout = cout
        self.expand = expand
        self.normstats = get_backbone_normstats('tf_efficientnet_lite0')
