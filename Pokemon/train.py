...
# Required.
@click.option('--outdir',help='Where to save the results', metavar='DIR',                default = './training-runs/')
@click.option('--cfg',help='Base configuration',              type=click.Choice(['fastgan', 'fastgan_lite', 'stylegan2']), default = 'fastgan_lite')
@click.option('--data',help='Training data', metavar='[ZIP|DIR]',type=str,default = '/home/user/Projects/pg-clc/pomkemon256.zip')
...
# Misc settings.
@click.option('--desc',help='String to include in result dir name', metavar='STR',     type=str,default = 'efficientnet-b0')
@click.option('--metrics',      help='Quality metrics', metavar='[NAME|A,B,C|none]',type=parse_comma_separated_list, default='fid50k_full', show_default=True)
@click.option('--kimg',help='Total training duration', metavar='KIMG',type=click.IntRange(min=1), default=10000, show_default=True)
...
@click.option('--dmodel',help='dmodel', metavar='STR', type=str, default='', show_default=True)
@click.option('--resume_kimg',help='Time interval in seconds to restart code', metavar='INT', type=int, default=0, show_default=True)
...
def main(**kwargs):
    ...
    c.G_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', betas=[0, 0.999], eps=1e-8)
    c.D_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', betas=[0, 0.999], eps=1e-8)
    ...
    # Closed Loop Control Loss
    c.loss_kwargs.clc_loss_type = 'square'
    c.loss_kwargs.clc_loss_weight = 0.1 # weight for the clc loss
    c.loss_kwargs.batch_size_queue = 64 # batch size of the queue
    c.loss_kwargs.factors=100 # number of factors of the queue
