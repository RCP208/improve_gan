...
def training_loop(
    ...
    best_fid = 9999
    # best_is = 0
    g_resume_opt_param = None
    d_resume_opt_param = None
    g_bank = None
    g_bank_ptr = None
    d_bank = None
    d_bank_ptr = None
    ...
    # apply dropout even it in eval mode
    def apply_dropout(m):
        if type(m) == nn.Dropout:
            m.train()
    G_ema.apply(apply_dropout)

    # Check for existing checkpoint
    ckpt_pkl = None
    if restart_every > 0 and os.path.isfile(misc.get_ckpt_path(run_dir, 1125)):
        ckpt_pkl = resume_pkl = misc.get_ckpt_path(run_dir, 1125)
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        if 'G_opt' in resume_data:
            g_resume_opt_param = resume_data['G_opt']
        if 'D_opt' in resume_data:
            d_resume_opt_param = resume_data['D_opt']
        # if 'g_bank' in resume_data:
        #     g_bank = resume_data['g_bank']
        # if 'g_ptr' in resume_data:
        #     g_bank_ptr = resume_data['g_ptr']
        # if 'd_bank' in resume_data:
        #     d_bank = resume_data['d_bank']
        # if 'd_ptr' in resume_data:
        #     d_bank_ptr = resume_data['d_ptr']
        # ema = resume_data['ema']
        # G_ema.load_state_dict(ema)
        ...
        if ckpt_pkl is not None:            # Load ticks
            ...
            print(f'Loaded progress from "{resume_pkl}"')
        del resume_data
    ...
    #     result_dict = metric_main.calc_metric(metric=metric, G=G_ema, run_dir=run_dir,
    ...
    if g_bank is not None and g_bank_ptr is not None:
        loss.g_queue.bank = g_bank
        loss.g_queue.bank_ptr = g_bank_ptr
    if d_bank is not None and d_bank_ptr is not None:
        loss.d_queue.bank = d_bank
        loss.d_queue.bank_ptr = d_bank_ptr
    phases = []
    ...
    for name, module, opt_kwargs, reg_interval in [('G', G, G_opt_kwargs, G_reg_interval) ,('D', D, D_opt_kwargs, D_reg_interval)]:
        if reg_interval is None:
            ...
            if name == 'G' and g_resume_opt_param is not None:
                opt.load_state_dict(g_resume_opt_param)
            elif name == 'D' and d_resume_opt_param is not None:
                opt.load_state_dict(d_resume_opt_param)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            all_gen_z = torch.randn([len(phases) * batch_size, G.z_dim], device=device) # [2 * 64, 256]
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            ...
            for phase in phases:
                if phase.name in ['Gmain', 'Gboth']:
                    snapshot_data['G_opt'] = phase.opt.state_dict()
                else:
                    snapshot_data['D_opt'] = phase.opt.state_dict()
            # if num_gpus > 1:
            #     snapshot_data['ema'] = G_ema.module.state_dict()
            # else:
            # snapshot_data['ema'] = G_ema.state_dict()

        # Save Checkpoint if needed
        if (rank == 0) and (restart_every > 0) and (network_snapshot_ticks is not None) and (
                done or cur_tick % network_snapshot_ticks == 0):
            snapshot_pkl = misc.get_ckpt_path(run_dir, name=cur_tick)
        ...
        if cur_tick and (snapshot_data is not None) and (len(metrics) > 0):
            ...
            # save best is IS ckpt
            # snapshot_pkl1 = os.path.join(run_dir, f'IS_best_model.pkl')
            # cur_nimg_txt1 = os.path.join(run_dir, f'IS_best_nimg.txt')
            # if rank == 0:
            #     if 'is50k' in stats_metrics and stats_metrics['is50k'] > best_is:
            #         best_is = stats_metrics['is50k']
            #
            #         with open(snapshot_pkl1, 'wb') as f:
            #             dill.dump(snapshot_data, f)
            #         # save curr iteration number (directly saving it to pkl leads to problems with multi GPU)
            #         with open(cur_nimg_txt1, 'w') as f:
            #             f.write(str(cur_nimg))
