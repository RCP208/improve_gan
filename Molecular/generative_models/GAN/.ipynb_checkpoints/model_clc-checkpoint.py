...
class MolGen(nn.Module):
    ...
    def train_step(self, x):
        ...
        # combined loss
        discr_loss = 0.5 * (real_loss + fake_loss) + (fake_clc + real_clc) * 0.1
        discr_loss.backward()
        ...
        pdb.set_trace()
        g_loss = F.binary_cross_entropy(y_pred, y_real, reduction='none') * y_pred_mask
        g_loss = g_loss.sum() / y_pred_mask.sum()
        mse_loss = F.mse_loss(x_gen, x_real, reduction='mean') * 0.1

        generator_loss = g_loss + mse_loss
